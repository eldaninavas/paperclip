...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Foundation will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Foundation handles this" mechanics bullets, composed from the
