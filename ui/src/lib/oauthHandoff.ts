...
function parseHandoff(value: unknown): { kind: "paperclip_cloud"; session: string } | null {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new OAuthHandoffError("Foundation Cloud returned an invalid sign-in handoff.", "invalid_handoff");
  }
...
function handoffFailure(status: number, code: unknown): OAuthHandoffError {
  ...
  if (status === 401 || status === 403) {
    return new OAuthHandoffError("Foundation Cloud could not authorize this connection for your account.", "forbidden");
  }
  return new OAuthHandoffError("Foundation Cloud couldn’t prepare secure sign-in. Try again.", "unavailable");
}
...
async function postCloudHandoff(
  ...
  throw new OAuthHandoffError(
    lastError instanceof Error ? lastError.message : "Foundation Cloud couldn’t prepare secure sign-in. Try again.",
    "unavailable",
  );
}
...
export async function prepareOAuthNavigation(
  ...
  if (!authorization.ok) {
    throw new OAuthHandoffError("Foundation Cloud returned an invalid provider sign-in address.", "invalid_handoff");
  }
...
export function savePendingCloudHandoff(
  ...
  if (!handoff) throw new OAuthHandoffError("Foundation Cloud returned an invalid sign-in handoff.", "invalid_handoff");
