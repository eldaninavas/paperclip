...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Foundation", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Foundation",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Foundation");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Foundation");
    expect(buildDocumentTitle([], null)).toBe("Foundation");
  });
});
