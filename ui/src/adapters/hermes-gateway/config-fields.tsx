...
export function HermesGatewayConfigFields({
  ...
  return (
    <>
      <Field
        label="API base URL"
        hint="Hermes API server base URL that Foundation can reach, such as http://127.0.0.1:8642 or a private HTTPS URL. Default dashboard root/chat URLs such as http://127.0.0.1:9119/chat are accepted and map to /api."
      >
        <DraftInput
          value={apiBaseUrl}
      ...
      <SecretField
        ...
        placeholder="Hermes API_SERVER_KEY, not the Foundation server key"
        stored={!isCreate && hasStoredApiKey && !editApiKeyValue}
      />

      <Field
        label="Foundation API URL"
        hint="Optional Foundation API URL reachable by the Hermes host. This is not a credential."
      >
        <DraftInput
