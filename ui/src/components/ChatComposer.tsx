...
import { AlertTriangle, Check, Loader2, Paperclip as Foundation, Send } from "lucide-react";
      ...
      {isDragOver && canAttach ? (
        ...
        >
          <span className="inline-flex items-center gap-2">
            <Foundation className="h-3.5 w-3.5" />
            Drop to attach
          </span>
        </div>
      ) : null}
      ...
      <div className="mt-2 flex items-center gap-2">
        {canAttach ? (
          <>
            ...
            >
              {attaching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Foundation className="h-4 w-4" />}
            </button>
          </>
        ) : null}
