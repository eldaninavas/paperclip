...
describe("ProjectTile", () => {
  it("renders a neutral gray cube tile by default", () => {
    ...
    // Cube icon is the default when no icon prop is supplied.
  ...
  it("falls back to the cube icon for an unknown icon name", () => {
    const markup = renderToStaticMarkup(<ProjectTile icon="not-a-real-icon" />);
    expect(markup).toContain("lucide-box");
  });

  it("normalizes the legacy folder default to the project cube", () => {
    const markup = renderToStaticMarkup(<ProjectTile icon="folder" />);
    expect(markup).toContain("lucide-box");
    expect(markup).not.toContain("lucide-folder");
  });

  it("renders a persisted project emoji", () => {
    const markup = renderToStaticMarkup(<ProjectTile icon="emoji:🚀" />);
