...
describe("CloudAccessGate", () => {
  ...
  it("shows browser sign-in setup for signed-out private bootstrap-pending instances", async () => {
    ...
    expect(container.textContent).toContain("Finish setting up this Foundation");
  ...
  it("keeps public bootstrap-pending instances invite-only", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "This Foundation is waiting on its first admin");

    expect(container.textContent).toContain("This Foundation is waiting on its first admin");
