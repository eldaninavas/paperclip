...
const MATERIALIZATION_NOTE =
  "Enabled skills are materialized into the stable Foundation-managed prompt bundle on the agent's next run.";

/** Company skill key of the Foundation core skill that carries beta releases. */
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const unsupportedMessage = useMemo(() => {
    ...
    if (
      ...
      return "Foundation cannot manage skills for custom ACP commands yet.";
    }
    if (agent.adapterType === "openclaw_gateway") {
      return "Foundation cannot manage OpenClaw skills here. Visit your OpenClaw instance to manage this agent's skills.";
    }
    return "Foundation cannot manage skills for this adapter yet. Manage them in the adapter directly.";
  ...
  const renderRow = (row: AgentSkillRowData, variant: "enabled" | "available") => {
    ...
    const rowDisabledReason = legacyPaperclipBlocked
      ? "Foundation Runner uses native semantic coordination and cannot attach the legacy Foundation operational skill."
      : unsupportedMessage;
