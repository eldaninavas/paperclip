...
function unavailableProvider(
  ...
  return {
    ...
    async createSecret() {
      throw unprocessable(`${id} provider is not configured for Foundation-managed values`);
    },
    async createVersion() {
      throw unprocessable(`${id} provider is not configured for Foundation-managed values`);
    },
    ...
    async deleteOrArchive() {
      // External references are metadata-only in Foundation for unconfigured providers.
    },
    async healthCheck() {
      return {
        provider: id,
