...
async function throwProviderWriteOrReservedRowRollbackError(input: {
  ...
  try {
    ...
    throw new HttpError(500, "Secret create failed and Foundation could not roll back the local secret reservation.", {
...
function throwProviderCleanupFailedAfterCreateRollback(input: {
  ...
  const providerConfigId = providerConfigIdentifier(input);
  throw new HttpError(500, "Secret create failed and Foundation could not clean up the remote provider secret.", {
...
function safeRemoteProviderErrorDetails(
  ...
  if (
    ...
    details.credentialPath = "Foundation server runtime/provider credential path";
    if (error?.code === "access_denied") {
      if (context.operation === "secret.create") {
        ...
        details.safeAlternative =
          "If the secret already exists in AWS, link it as an external reference instead of creating a Foundation-managed value.";
