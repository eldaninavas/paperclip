...
export const DEFAULT_STRANDED_RECOVERY_NOTICE_BODY =
  "Foundation could not restore a live execution path for this issue automatically. " +
...
export function buildImmediateExecutionPathRecoveryNoticeSeed(input: {
  ...
  const retryDescription = input.status === "todo"
    ? "Foundation automatically retried dispatch for this assigned `todo` issue during terminal run recovery"
    : "Foundation automatically retried continuation for this assigned `in_progress` issue during terminal run recovery";
  return {
    body:
...
export function buildWorkspaceValidationRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Foundation stopped before launching the local adapter because the issue workspace failed validation. " +
...
export function buildConfigurationIncompleteRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Foundation stopped before dispatching the adapter because required secret/env bindings are missing. " +
...
export function buildExecutionReviewParticipantRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Foundation retried the pending execution-review participant once, but the review stage still has no " +
...
export function buildExecutionReviewParticipantUnavailableNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Foundation cannot continue the pending execution-review participant because the participant is not " +
